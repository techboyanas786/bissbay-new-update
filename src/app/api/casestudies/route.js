...
export async function POST(request) {
    ...
    try {
        ...
        return NextResponse.json(
            { 
                success: true,
                message: 'Case studies form submitted successfully',
                id: result.insertId 
            },
            { status: 201 }
        );
        ...
        return NextResponse.json(
            { error: 'Failed to submit case studies form. Please try again later.' },
            { status: 500 }
        );
    } finally {
        if (connection) {
...
export async function GET(request) {
    try {
        ...
        return NextResponse.json(
            { error: 'Failed to fetch case studies submissions' },
            { status: 500 }
        );
    }
}